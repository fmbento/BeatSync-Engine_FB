...
def build_planned_clip_sequence(
    ...
    variety = float(beat_info.get("variety", 0.5))
...
def _choose_candidate(
    ...
    # Scale the reuse penalties and randomness by variety.
    # Base: 0.5 variety is the legacy behavior.
    variety_scale = max(0.01, variety * 2.0)
    ...
    for candidate in candidates:
        ...
        if cid in recent_ids:
            score -= 0.28 * variety_scale
        if video_file in recent_videos:
            score -= 0.10 * variety_scale
        score -= min(0.28, usage[cid] * 0.10) * variety_scale
        score -= min(0.18, usage[video_file] * 0.012) * variety_scale
        ...
        score += rng.random() * 0.015 * variety_scale
        if score > best_score:
            best_score = score
