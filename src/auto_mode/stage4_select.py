...
def select_section_wave_cuts(beat_indices: np.ndarray, beat_times: np.ndarray,
    ...
    while current_pos < beat_indices.size - 1:
        ...
        step = adaptive_beat_step(wave, impact, section_type, pattern, cfg)
...
def adaptive_beat_step(wave: float, impact: float, section_type: str, pattern: str, cfg: AutoWaveConfig) -> int:
    ...
    elif section_type in {"drop", "finale"}:
