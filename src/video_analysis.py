...
def analyze_video_sources(
    ...
    print(f"   Analysis tuning: variety={variety:.2f}, min_beats={min_beats}")
...
def _analyze_single_video(
    ...
    step_started = time.perf_counter()
    # Dynamic threshold: higher variety = much lower threshold (more sensitive)
    # 0.5 variety -> 0.27 (default)
    # 1.0 variety -> 0.08 (very aggressive)
    scene_threshold = 0.27 * (1.3 - variety)
    ...
    print(
        f"      ⏱ Scene detection (thr={scene_threshold:.3f}) total: {_fmt_seconds(timings['scene_detection_seconds'])} "
        f"({len(scene_changes)} scene cuts)"
    )

    step_started = time.perf_counter()
    boundaries = _build_boundaries(scene_changes, duration)
    windows = _make_candidate_windows(boundaries, duration, variety=variety)
...
def _complete_deferred_qwen_batch(
    ...
    max_windows_base = int(os.environ.get("BEATSYNC_QWEN_MAX_WINDOWS", "120"))
    # Scale max_windows with variety: 1.0 variety -> up to 360 windows
    max_windows = int(max_windows_base * (0.8 + variety * 2.2))
...
def _build_boundaries(scene_changes: Iterable[float], duration: float) -> List[float]:
    ...
    cleaned: List[float] = []
    # min_gap depends on variety? Not strictly required but let's keep it tight.
    for t in raw:
        if not cleaned or t - cleaned[-1] >= 0.15 or t in (0.0, duration):
            cleaned.append(t)
...
def _make_candidate_windows(boundaries: Sequence[float], duration: float, variety: float = 0.5) -> List[Dict]:
    windows: List[Dict] = []
    if duration <= 0:
        return windows

    # Tuning based on variety
    # High variety -> shorter windows, more frequent fallback sampling
    max_window = 5.2 * (1.1 - variety * 0.4) # 1.0 variety -> 3.6s max window
    min_window = 0.55 * (1.1 - variety * 0.5) # 1.0 variety -> 0.33s min window
    fallback_step = 3.5 * (1.1 - variety * 0.6) # 1.0 variety -> 1.75s step
